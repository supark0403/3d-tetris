...
func _eval_clear(tspin: bool) -> void:
	var full: Array = []
	for r in ROWS:
		var ok := true
		for c in COLS:
	...
	if n == 0:
		if tspin:
			# T스핀 노라인도 점수+콤보 유지
			combo += 1
			var pts := TSPIN_SCORE[0] * level
			if b2b:
				pts = int(pts * 1.5)
			b2b = true
			score += pts
			_sfx("tspin")
			emit_signal("announce", "T-SPIN!", true)
			emit_signal("cleared", [], "T-SPIN", pts)
		else:
			combo = -1
		emit_signal("stats_changed")
		return
	...
	var label := ""
	var pts := 0
	var difficult := n == 4 or tspin
	if tspin:
		pts = TSPIN_SCORE[mini(n, 3)] * level
		label = "T-SPIN " + (["", "SINGLE", "DOUBLE", "TRIPLE"][mini(n, 3)] if n > 0 else "")
	else:
